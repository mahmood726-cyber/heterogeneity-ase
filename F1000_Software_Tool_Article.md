# New Heterogeneity Model: a software tool for reproducible evidence-synthesis and modeling workflows
...
## Abstract
**Background:** Reproducible evidence-synthesis model development often requires repeated fitting, heterogeneity checks, and structured reporting outputs. New Heterogeneity Model was prepared to support these workflows in a traceable software package.
...
**Conclusions:** New Heterogeneity Model provides a practical workflow for transparent modeling and evidence synthesis; external submission readiness depends on final public repository metadata and DOI-archived release records.

## Keywords
heterogeneity modeling; evidence synthesis; reproducibility; model validation; software tool
...
- Load demonstration or test data (example reference: `data\output\unified_hta_validation.csv`).
...
| Validation scope | Validation, test, or benchmark artifacts were identified for package-level checking. | `benchmark_ase.R` |
...
- Dataset reference: `data\output\unified_hta_validation.csv`
...
2. Load a demonstration/test dataset (e.g., `data\output\unified_hta_validation.csv`).
...
## Conclusions
New Heterogeneity Model is structured for reproducible modeling workflows with explicit evidence mapping and bounded claims. With final repository/DOI metadata completed, the package is prepared for software-tool reporting.
...
|---|---|---|---|
| Example walkthrough dataset | `data\output\cleaned_rds\CD000028_pub4_data.rds` | Present | Verify rerun on clean machine |
...
## Data availability
No new participant-level clinical data were generated for this software article package. Example dataset for reviewer walkthrough: `data\output\cleaned_rds\CD000028_pub4_data.rds`. Additional project data assets, where present, remain available within the local package tree.
